...
class AsymmetricMaskedCroCoMulti(CroCoNet):
    ...
    def _encode_image(self, image, true_shape, intrinsics_embed=None, intrinsics_pos_embed=None, pose_embedding=None):
        ...
        # Keep four evenly spaced encoder features for downstream DPT fusion.
        # Views are still folded into the batch here, so these features have not
        # passed through the multi-view decoder.
        feature_hooks = {
            (i + 1) * len(self.enc_blocks) // 4 - 1
            for i in range(4)
        }
        encoder_features = []
        for block_idx, blk in enumerate(self.enc_blocks):
            x = blk(x, pos)
            if block_idx in feature_hooks:
                encoder_features.append(x)

        x = self.enc_norm(x)
        encoder_features[-1] = x
        return x, pos, P, encoder_features
    ...
    def forward(self,
        ...
        feat, pos, P, enc_feat = self._encode_image(
            images_all,
            shape_all,
            intrinsic_embedding_all,
            pose_embedding=pose_embedding_all,
        )  # enc_feat: 4 x (bv, n_tokens, 1024)
        # ****************** decoder *******************************
        ...
        enc_feat = [
            rearrange(token[:, :P], "(b v) l c -> b v l c", b=b, v=v)
            for token in enc_feat
        ]
