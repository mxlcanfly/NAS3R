...
class AsymmetricMaskedCroCoMulti(CroCoNet):
    ...
    def encode_views(self, context: dict) -> dict:
        """Run the per-image ViT encoder without any multi-view decoding."""
    ...
    def decode_views(
            self,
            encoded: dict,
            feature_override: Optional[torch.Tensor] = None,
            target_num_views: int = 0,
    ) -> dict:
        """Run the original CroCo multi-view decoder on encoded image tokens."""
        feat = encoded["feat"]
        P = encoded["patch_tokens"]
        if feature_override is not None:
            if feature_override.shape != feat[:, :, :P].shape:
                raise ValueError(
                    "feature_override must match image-token shape, got "
                    f"{tuple(feature_override.shape)} and {tuple(feat[:, :, :P].shape)}."
                )
            feat = torch.cat([feature_override, feat[:, :, P:]], dim=2)

        dec_feat = self._decoder(
            feat,
            encoded["pos"],
            intrinsics_embed=encoded["intrinsic_embedding"],
            pose_embedding=encoded["pose_embedding"],
            num_target=target_num_views,
        )
        pose_feat = None
        if self.pose_embed_loc != 'none' and self.pose_embed_type == 'learnable_token':
            pose_feat = []
        ...
        out = {
            ...
            "encoder_image_tokens": encoded["image_tokens"],
            "encoder_positions": encoded["pos"][:, :, :P],
        }
    ...
    def forward(self,
        ...
        encoded = self.encode_views(context)
        return self.decode_views(encoded, target_num_views=target_num_views)
