...
class BackboneMaskedVGGT(Backbone[BackboneMaskedVGGTCfg]):
    def __init__(self, cfg: BackboneMaskedVGGTCfg, d_in: int) -> None:
        ...
        if self.cfg.pretrained:
            self.model = VGGT.from_pretrained("/datasets2/yuhc/modelzoo/VGGT/VGGT-1B/",
                                              intrinsics_embed_loc=self.cfg.intrinsics_embed_loc,
                                              depth_activation=self.cfg.depth_activation)
        else:
        ...
        if self.cfg.init_intrinsics:
            ...
            nn.init.constant_(self.model.camera_head.pose_branch.fc2.bias[-2:],
                              2 * torch.atan(torch.tensor(0.5)) / self.cfg.num_iterations)
    ...
    def forward(
        ...
        # context + target
        aggregated_tokens_list, ps_idx = self.model.aggregator._decoder_with_mask(patch_tokens, h, w,
                                                                                  context["intrinsics"],
