...
def rescale(
    ...
    image_new = image_new.resize((w, h), resample)
    image_new = np.array(image_new) / 255
    image_new = torch.tensor(image_new, dtype=image.dtype, device=image.device)
    return rearrange(image_new, "h w c -> c h w").clamp(0, 1)
...
def downsample_bicubic(
    ...
    *batch, c, h, w = images.shape
    ...
    images = torch.stack(
        [rescale(image, shape, resample=Image.BICUBIC) for image in images]
    )
    h_out, w_out = shape
    return images.reshape(*batch, c, h_out, w_out)
...
def apply_crop_shim_to_views(views: AnyViews, shape: tuple[int, int]) -> AnyViews:
    ...
    images_lr = downsample_bicubic(images, (64, 64))
